<?php  
	...
	if ($db) {
		...
		// nombre, autor, seccion, fecha, correo, descripcion
		...
		// Validar datos con JavaScript. falta ??
		...
		if ($confirmaciones == 8) {	// Todos los campos están rellenos
			...
			if ($num > 0) {
				// echo "Ya existe otro usuario con ese alias. Pruebe de nuevo.", PHP_EOL;
				$user_repetido = true;
			}
			else{
				...
				if($consulta){ // Nuevo usuario añadido
					$check = true;
					//echo "El insert ha tenido exito", PHP_EOL;
				}

				//else{ // Inserción fallida
				//	echo "El insert ha fallado", PHP_EOL;
				//}
			}
		}
	...
	function bodyRegistro($usuario="", $confirmacion, $usuario_repetido){
		...
		if ($confirmacion == true) {
			...
			<p> El usuario $usuario ha quedado registrado, pulse para iniciar sesión.</p>
